...
class Model {
	...
	public function filterArrayToString($filter){
		...
		foreach ($filter as $key => $value) {
			...
			array_push($urlFilterArray, $key . '=' . urlencode($value));
		}
	...
	public function getArtefactID($videoId){

		$db = $this->db->useDB();
		$collection = $this->db->selectCollection($db, ARTEFACT_COLLECTION);

		$result = $collection->findOne(['videoId' => $videoId]);

		return $result['id'];
	}

	public function getSutradhar($referenceYear){

		$db = $this->db->useDB();
		$collection = $this->db->selectCollection($db, ARTEFACT_COLLECTION);

		$match = [ 'referenceYear' => intval($referenceYear), 'sutradhar' => 'Yes'];

		$projectArray['Type'] = 1;
		$projectArray['interviewee'] = 1;
		$projectArray['dataType'] = 1;
		$projectArray['title'] = 1;
		$projectArray['sutradhar'] = 1;
		$projectArray['referenceYear'] = 1;
		$projectArray['id'] = 1;

		$iterator = $collection->findOne($match,['projection' => $projectArray]);

		$iterator['idURL'] = str_replace('/', '_', $iterator['id']);
		$iterator['link'] = BASE_URL . "describe/dartefact/" . $iterator['idURL'] . '?year=' . $referenceYear;
